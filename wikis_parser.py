...
def set_shards_number_to_db(curr_shard, wikis, user_db_port=None, user=None, password=None):
    ...
    try:
        ...
        with conn.cursor() as cur:
            for wiki in wikis:
                cur.execute(query, [curr_shard, wiki])
        conn.commit()
        conn.close()
    except Exception as err:
        print("Something went wrong.\n", err)
...
def get_database_shards_info(user_db_port=None, user=None, password=None):
    ...
    for i in range(constants.SHARDS_AMOUNT):
        ...
        try:
            page = requests.get(constants.NOC_DATABASES_DIRECTORY + shard_name)
            page.raise_for_status()
            soup = BeautifulSoup(page.text, 'html.parser')
            curr_set = soup.string.strip().split("\n")[1:]          # first entry is autogenerated string
            set_shards_number_to_db(i + 1, curr_set, user_db_port, user, password)
        except requests.exceptions.HTTPError:
            print("Looks like page with name %s does not exist." 
                  "Please check https://noc.wikimedia.org/conf/ and change shards amount" % shard_name)
